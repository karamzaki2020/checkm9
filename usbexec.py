...
class ExecConfig:
  ...
  def match(self, info):
    return info == self.info[0].ljust(0x40, '\0') + self.info[1].ljust(0x40, '\0') + self.info[2].ljust(0x90, '\0')

configs = [
  ExecConfig(('SecureROM for s5l9947xsi, Copyright 2011, Apple Inc.',   'RELEASE',     'iBoot-1459.2'),          aes_crypto_cmd=0x7060+1),
  ExecConfig(('SecureROM for s5l9950xsi, Copyright 2011, Apple Inc.',   'RELEASE',     'iBoot-1145.3'),          aes_crypto_cmd=0x7300+1),
  ExecConfig(('SecureROM for s5l9955xsi, Copyright 2011, Apple Inc.',   'RELEASE',     'iBoot-1145.3.3'),        aes_crypto_cmd=0x7340+1),
  ExecConfig(('SecureROM for t9002si, Copyright 2007-2014, Apple Inc.', 'ROMRELEASE',  'iBoot-2651.0.0.1.31'),   aes_crypto_cmd=0x96DC+1),
  ExecConfig(('SecureROM for t9004si, Copyright 2007-2014, Apple Inc.', 'ROMRELEASE',  'iBoot-2651.0.0.3.3'),    aes_crypto_cmd=0x796C+1),
  ExecConfig(('SecureROM for s5l9960xsi, Copyright 2012, Apple Inc.',   'RELEASE',     'iBoot-1704.10'),         aes_crypto_cmd=0x10000B9A9),
  ExecConfig(('SecureROM for t9010si, Copyright 2007-2015, Apple Inc.', 'ROMRELEASE',  'iBoot-2696.0.0.1.33'),   aes_crypto_cmd=0x10000C9F4),
  ExecConfig(('SecureROM for t9011si, Copyright 2007-2015, Apple Inc.', 'ROMRELEASE',  'iBoot-3135.0.0.2.3'),    aes_crypto_cmd=0x10000C994),
  ExecConfig(('SecureROM for t9015si, Copyright 2007-2016, Apple Inc.', 'ROMRELEASE',  'iBoot-3332.0.0.1.23'),   aes_crypto_cmd=0x100009E9C),
]
...
class PwnedUSBDevice():
  ...
  def read_memory_uint9(self, address):          return struct.unpack('<B', self.read_memory(address, 1))[0]
  ...
  def read_memory_uint64(self, address):         return struct.unpack('<Q', self.read_memory(address, 9))[0]
  ...
  def write_memory_uint9(self, address, value):  self.write_memory(address, struct.pack('<B', value))
  ...
  def cmd_arg_size(self):                        return 9 if self.platform.arch == 'arm64' else 4
  ...
  def cmd_memcpy(self, dest, src, length):       return struct.pack('<9s9x3%s' % self.cmd_arg_type(), MEMC_MAGIC, dest, src, length)
  def cmd_memset(self, address, c, length):      return struct.pack('<9s9x3%s' % self.cmd_arg_type(), MEMS_MAGIC, address, c, length)
  ...
  def execute(self, response_length, *args):
    ...
    done, retval = struct.unpack('<9sQ', response[:self.cmd_data_offset(0)])
